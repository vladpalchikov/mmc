        ...
        @if ($service->service->getCompanyComplex($company->id))
            <div class="document-preview">
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            Прошу Вас вернуть денежные средства в размере <span class="field capitalize">{{ number_format($service->service->getCompanyComplex($company->id), 0, ',', ' ') }}</span> рублей (<span class="field">{{ trim(Helper::num2str($service->service->getCompanyComplex($company->id))) }}</span>) рублей по заявке №{{ $service->id }} на услугу "<span class="field">{{ $service->service->description }}</span>" от <span class="field">{{ Helper::ru_date('%e %b %Y', strtotime($service->created_at)) }} г.</span><br><br>
                            <div class="und">&nbsp;</div>
                            <div style="text-align:center">(причина возврата)</div>
                            <br>
                            <p>Приложение:</p>
                            <span class="check">&nbsp;</span>Кассовый чек<br>
...
@else
    <div class="document-preview">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-md-12">
                    Прошу Вас вернуть денежные средства в размере <span class="field capitalize">{{ number_format($service->service_price, 0, ',', ' ') }}</span> рублей (<span class="field">{{ trim(Helper::num2str($service->service_price)) }}</span>) рублей по заявке №{{ $service->id }} на услугу "<span class="field">{{ $service->service->description }}</span>" от <span class="field">{{ Helper::ru_date('%e %b %Y', strtotime($service->created_at)) }}г.</span><br><br>
                    <div class="und">&nbsp;</div>
                    <div style="text-align:center">(причина возврата)</div>
                    <br>
                    <p>Приложение:</p>
