...
@section('title', 'Принимающие стороны')

@section('menu')
    @include('partials.operator-menu', ['active' => 'registrationreport'])
@endsection

@section('content')
    <div class="row">
        <div class="col-md-12">
            <h3 class="pull-left main-title main-title-single">Отчет о принимающих сторонах</h3>
            <a href="/operator/report/registration/print?daterange={{ Request::get('daterange') }}&@if (Request::has('manager'))&manager={{ Request::get('manager') }}@endif&@if (Request::has('client'))&client={{ Request::get('client') }}@endif" class="btn btn-default pull-right">Распечатать</a>
        </div>
    </div>
                        ...
                        <div class="form-group">
                            <div class="input-group">
                                <select name="client" class="form-control pull-right">
                                    ...
                                    @foreach($clientsForFilter as $client)
    ...
    @if ($clients->count() > 0)
        ...
        @include('report.registration.table')
    @else
