            ...
            @foreach ($servicesByOperator as $row)
                <tr>
                    ...
                    <td class="text-right nbr" style="position: relative">
                        @if ($row['operator']->role == 'Ст. Менеджер ТМ' || $row['operator']->role == 'Менеджер ТМ')
                            {{ ($row['totalPayment'] / 3) + $row['totalPaymentComplex'] + ($row['totalDocuments'] * 3.2) }}
                            <div style="position: absolute; bottom: -1px; left:0; background-color: #1abc9c; width: {{ (($row['totalPayment'] / 3) + $row['totalPaymentComplex'] + ($row['totalDocuments'] * 3.2))/1680*100 }}%; height: 3px"></div>
                        @else
                            {{ $row['totalPayment'] }}
                            <div style="position: absolute; bottom: -1px; left:0; background-color: #1abc9c; width: {{ $row['totalPayment']/1680*100 }}%; height: 3px"></div>
                        @endif
                    </td>
                    ...
                    <td class="text-right nbr">
                        @if ($row['operator']->role == 'Ст. Менеджер ТМ' || $row['operator']->role == 'Менеджер ТМ')
                            @if (((($row['totalPayment'] / 3) + $row['totalPaymentComplex'] + ($row['totalDocuments'] * 3.2)) - 1680) * 30 < 0)
                                0
                            @else
                                {{ ((($row['totalPayment'] / 3) + $row['totalPaymentComplex'] + ($row['totalDocuments'] * 3.2)) - 1680) * 30 }}
                            @endif
                        @elseif ($row['operator']->role == 'Ст. Менеджер МУ' || $row['operator']->role == 'Менеджер МУ')
                            @if ((($row['totalPayment'] + $row['totalPaymentComplex'] + ($row['totalDocuments'] * 3.2)) - 1680) * 14 < 0)
                                0
                            @else
                                {{ (($row['totalPayment'] + $row['totalPaymentComplex'] + ($row['totalDocuments'] * 3.2)) - 1680) * 14 }}
                            @endif
                        @endif
                    </td>
                </tr>
