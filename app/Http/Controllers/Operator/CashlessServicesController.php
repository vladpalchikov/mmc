...
class CashlessServicesController extends Controller
{
	public function index(Request $request)
    {
        ...
        if ($request->has('module')) {
        	...
        	$services = ForeignerService::where('is_mu', 0)->where('repayment_status', 0);
    	}

    	$services = $services->where('payment_status', 0)->where('payment_method', 1);
		...
		return view('operator.foreigner.cashless-services', compact('services', 'clients'));
	}
    ...
    public function pay(Request $request)
    {
        if ($request->has('id') && $request->has('type')) {
            ...
            if ($type == 'tm') {
                ...
                $balance = $service->service->company->getBalance($client);

                if ($balance < $service->service_price) {
                    return response()->json(['status' => 'error', 'data' => 'Недостаточно средств на балансе плательщика']);
                }
                ...
                $service->save();
                return response()->json(['status' => 'success', 'client_id' => $service->client_id, 'company_id' => $service->service->company->id, 'balance' => number_format($balance, 0, ',', ' ')]);
            } else {
                ...
                $balance = $group->service->company->getBalance($client);

                if ($balance < $group->service_price * $group->service_count) {
                    return response()->json(['status' => 'error', 'data' => 'Недостаточно средств на балансе плательщика']);
                }
                ...
                return response()->json(['status' => 'success', 'client_id' => $group->client_id, 'company_id' => $group->service->company->id, 'balance' => number_format($balance, 0, ',', ' ')]);
            }
        }
    }
}
